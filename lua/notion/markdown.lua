...
local function onSave()
  if vim.api.nvim_buf_get_var(0, 'owner') ~= 'notionJson' then
    return
  end
  local new = string.gsub(
    require('notion').readFile(
      vim.fn.stdpath('data') .. '/notion/tempJson.json'
    ),
    '\n',
    ''
  )
  ...
  local id =
    require('notion').readFile(vim.fn.stdpath('data') .. '/notion/id.txt')
  if type == 'page' then
    local temp = {}
    local i = 1
    ...
    local window =
      require('notion.window').create('Saving: ' .. 0 .. '/' .. #temp)
    vim.fn.timer_start(1000, function()
      ...
      require('notion').writeFile(
        vim.fn.stdpath('data') .. '/notion/currentJob.txt',
        '\nSaving data: ' .. i .. '/' .. #temp
      )
      i = i + 1
      if i == #temp + 1 then
  ...
  elseif type == 'databaseEntry' then
    ...
    require('notion.request').savePage(
      '{"properties": ' .. vim.json.encode(data) .. '}',
      id,
      window
    )
  end
...
local function createFile(text, _, id)
  ...
  vim.schedule(function()
    ...
    end, require('notion').opts.delays.format)
...
M.page = function(data, id, _, open)
  ...
  local function onChild(child)
    ...
    require('notion').writeFile(
      vim.fn.stdpath('data') .. '/notion/tempJson.json',
      vim.json.encode(toWrite)
    )
    require('notion').writeFile(
      vim.fn.stdpath('data') .. '/notion/staticJson.json',
      vim.json.encode(toWrite)
    )
    vim.schedule(function()
    ...
    local function parseBlocks(blocks)
      ...
      for _, block in ipairs(blocks) do
        ...
        if block.type == 'heading_1' then
          markdown = markdown
            .. '# '
            .. parseRichText(block.heading_1.rich_text)
            .. '\n\n'
          prevBlock = block.type
        elseif block.type == 'heading_2' then
          markdown = markdown
            .. '## '
            .. parseRichText(block.heading_2.rich_text)
            .. '\n\n'
          prevBlock = block.type
        elseif block.type == 'heading_3' then
          markdown = markdown
            .. '### '
            .. parseRichText(block.heading_3.rich_text)
            .. '\n\n'
          prevBlock = nil
        elseif block.type == 'paragraph' then
          markdown = markdown
            .. parseRichText(block.paragraph.rich_text)
            .. '\n\n'
          prevBlock = nil
        ...
        elseif block.type == 'toggle' then
          markdown = markdown
            .. '<details><summary>'
            .. parseRichText(block.toggle.rich_text)
            .. '</summary>\n'
          markdown = markdown
            .. parseBlocks(block.toggle.children)
            .. '</details>\n\n'
        end
      end
      return markdown
    ...
    vim.schedule(function()
      require('notion').writeFile(
        vim.fn.stdpath('data') .. '/notion/temp.md',
        ftext .. '\n\n' .. markdown
      )
      if open then
...
M.databaseEntry = function(data, id, silent, open)
  ...
  for i, v in pairs(data.properties) do
    ...
    elseif
      v.type == 'select'
      and v.select ~= nil
      and v.select ~= vim.NIL
      and v.select.name ~= nil
      and v.select.name ~= vim.NIL
    then
