...
local viewItem = function(prompt_bufnr)
  ...
  if data.object == 'database_id' then
    markdown =
      markdownParser.databaseEntry(data.result, selection.value.id, true, true)
...
local remind = function(prompt_bufnr)
  ...
  if not file then
    return vim.notify('[Notion] Setup is incomplete')
  end
  file:write(
    date .. 'T' .. min_hour .. ' ' .. selection.value.displayName .. '\n'
  )
