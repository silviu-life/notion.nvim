...
local function checkReminders()
  local reminders =
    M.readFile(vim.fn.stdpath('data') .. '/notion/reminders.txt')
  ...
  for _, k in ipairs(lines) do
    ...
    local difference = os.difftime(
      os.time { year = Y, month = Mo, day = D, hour = H, min = min },
      os.time()
    )
    if difference < 60 then
      ...
      vim.defer_fn(function()
        require('notion.window').close(win)
      end, 5000)
    else
  ...
  require('notion').writeFile(
    vim.fn.stdpath('data') .. '/notion/reminders.txt',
    table.concat(final, '\n')
  )
end
...
M.setup = function(opts)
    ...
    complete = function(ArgLead, _, _)
      ...
      if ArgLead == '' or ArgLead == nil or ArgLead == ' ' then
        ...
        return {
          closest_match(
            ArgLead,
            { 'clear', 'update', 'setup', 'menu', 'status' }
          ),
        }
      end
    end,
  ...
  if M.opts.autoUpdate then
    ...
    vim.fn.timer_start(M.opts.delays.update, function()
      M.update { silent = true, window = nil }
    end, { ['repeat'] = -1 })
    vim.fn.timer_start(M.opts.delays.reminder, function()
      checkReminders()
    end, { ['repeat'] = -1 })
  end
end
...
M.status = function()
  ...
  local str = '[Notion] Last Update: '
    .. os.difftime(os.time(), M.lastUpdate)
    .. ' seconds ago'
  local currentJob =
    M.readFile(vim.fn.stdpath('data') .. '/notion/currentJob.txt')
  str = str .. currentJob
  vim.print(str)
end
